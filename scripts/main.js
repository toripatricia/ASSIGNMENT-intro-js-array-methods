...
const search = (event) => {
  ...
  const result = referenceList.filter(item => 
...
const buttonFilter = (event) => {
  ...
  if(event.target.id.includes('cartFilter')) {
    const cart = referenceList.filter(item => item.inCart === true) // === true not needed but i like it in there bc it's clear what i'm filtering
    renderCards(cart);
  }

  if(event.target.id.includes('books')) {
    const books = referenceList.filter(item => item.type.toLowerCase() === "book");
    // could also have written: (item => item.type === "Book");
    // .toLowerCase will target a string/value no matter the case you enter
    renderCards(books);
  }

  if(event.target.id.includes('clearFilter')) {
    renderCards(referenceList);
    // because we just want clear filter to return all cards
  }
